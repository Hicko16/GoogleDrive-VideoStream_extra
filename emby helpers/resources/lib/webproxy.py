    ...
    def do_GET(self):
        ...
        if re.search(r'/testlock/', str(self.path)):
            ...
            if results:
                ...
                request = urllib2.Request(self.server.serverURL + '/emby/Users/New?api_key=' + self.server.key, "{\"Name\":\""+str(self.server.label) +str(username)+"\"}",{'Content-Type': 'application/json'})
                ...
                if results:
                    ...
                    response = urllib2.urlopen(request)
                    request = urllib2.Request(self.server.serverURL + '/emby/Users/'+str(ID)+'/Password?api_key=' + self.server.key, "{\"Id\":\""+str(self.server.label)+str(username)+"\",\"CurrentPassword\":\"\",\"CurrentPw\":\"\",\"NewPw\":\""+str(password)+"\"}",{'Content-Type': 'application/json'})
                    response = urllib2.urlopen(request)
                    try:
                        ...
                        self.wfile.write('ID created and linked to connect ID.  Manual login details are: ' + str(self.server.manual) + ', username = ' + str(self.server.label)+str(username) + ', password = ' + str(password))
                    except:
                        self.wfile.write('Local (manual) ID created but linking to connect ID failed.  Manual login details are: ' + str(self.server.manual) + ', username = ' + str(self.server.label)+str(username) + ', password = ' + str(password))
