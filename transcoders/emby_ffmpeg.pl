...
my $FFMPEG_TEST = CONFIG->FFMPEG_OEM; #CONFIG->PATH_TO_EMBY_FFMPEG.'/ffmpeg.oem ';
...
# in Emby 3.5.2 +, DVR requests mimic Live TV requests (they no longer use the -d for length of time to record)
	...
	if ($arglist =~ m%$IPTV_DETERMINATOR%){
		...
		if ($arglist =~ m%\-pix_fmt yuv420p%){
			$arglist =~ s%\-codec\:v\:0 .* -f segment%\-codec\:v\:0 copy \-copyts \-vsync \-1 \-codec\:a\:0 copy \-copypriorss\:a\:0 0 \-f segment%;
			#emby4.0
			$arglist =~ s%\-map_metadata .* -f segment%\-codec\:v\:0 copy \-copyts \-vsync \-1 \-codec\:a\:0 copy \-copypriorss\:a\:0 0 \-f segment%;
		}
...
#			`$FFMPEG_TEST -http_proxy $PROXY $arglist -v error`;
	        ...
	        if (CONFIG->IPTV_MANAGE_SERVER ne ''){
	        	my $url = CONFIG->IPTV_MANAGE_SERVER.'/free/'. $username . '/'.$m3u8;
				`echo "FETCHED $url" >> /tmp/transcode.log;$FFMPEG_TEST $arglist -v error; wget "$url";echo "FREED $url" >> /tmp/transcode.log`;
